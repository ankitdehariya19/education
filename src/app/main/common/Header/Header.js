    ...
    <>
      ...
      <header className="shadow-2xl bg-white md:shadow-blue-500/20 md:mr-16 md:ml-16 px-8">
        <div className="md:bg-transparent md:mx-4 md:px-4 flex justify-between items-center relative md:mr-40 md:ml-40">
          <nav className={`hidden md:flex justify-between items-center ${click ? 'hidden' : ''}`}>
            <ul className="md:flex space-x-6 ml-6">
              {navLinks.map((link) => (
                <li
                  key={link.href}
                  className="text-black font-medium transition duration-300 hover:text-teal-800"
                >
                  <ScrollLink
                    to={link.href}
                    spy={true}
                    ...
                    onSetActive={closeMobileMenu} // Close mobile menu after scrolling to section
                  >
                    {link.label}
                  </ScrollLink>
                </li>
          ...
          <button className="toggle md:hidden absolute top-0 right-0 mt-2 mr-4" onClick={() => setClick(!click)}>
            {click ? <FaTimes size={30} /> : <FaBars size={30} />}
          </button>
          {click && (
            <div className="mobile-nav md:hidden fixed inset-0 bg-teal-500 z-40 animate-fadeIn" onClick={() => setClick(false)}>
              <ul className="z-50 flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-teal-500 text-gray-500">
                {navLinks.map((link) => (
                  <li
                    key={link.href}
          ...
          <div className={`start bg-teal-500 p-4 md:p-6 ${click ? 'md:ml-auto' : ''}`}>
            <div className="button text-white">GET CERTIFICATE</div>
          </div>
        </div>
      </header>
    </>
